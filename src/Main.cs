...
using System.Text;
using HarmonyLib;
using Kiner.ADOFAIAudioSync.Patches;
...
namespace Kiner.ADOFAIAudioSync
{
    public static class Main
    {
        ...
        private static void TryInstallPatches(UnityModManager.ModEntry modEntry)
        {
            ...
            TryPatchClass(typeof(ConductorCheckpointFreezePatch));
            TryPatchClass(typeof(AudioManagerStopLifecyclePatch));
            TryPatchClass(typeof(ConductorKillSoundsLifecyclePatch));
            TryPatchClass(typeof(ControllerRestartLifecyclePatch));
            TryPatchClass(typeof(ControllerFailLifecyclePatch));
            TryPatchClass(typeof(CheckpointCountdownFoldPatch));
            TryPatchClass(typeof(WaitBeatsCountdownIsolationPatch));
            TryPatchClass(typeof(CountdownMultiplierReleasePatch));
        ...
        private static bool TryPatchClass(Type patchType)
        {
            ...
            {
                LogException("Harmony patch failed: " + patchType.FullName, ex);
                return false;
            }
        }
        ...
        private static void NormalizeSettings()
        {
            ...
            {
                // Older play-error correction applied the full measured factor. Use a damped
                // first pass while keeping the corrected BPM active for following floors.
            ...
            if (Settings.SettingsRevision < 920)
            {
        ...
        {
            Enabled = value;
            AudioSyncRuntime.Reset(value ? "Mod有効" : "Mod無効", !value);
        ...
        private static void OnGUI(UnityModManager.ModEntry modEntry)
        {
            ...
            GUILayout.Label("開始確認に必要な音声更新回数: " + Settings.CheckpointStartStableFrames);
