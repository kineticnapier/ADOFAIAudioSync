    ...
    public sealed class AudioSyncBehaviour : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (control && Input.GetKeyDown(KeyCode.F9))
                Main.Settings.ShowOverlay = !Main.Settings.ShowOverlay;
        ...
        private void OnGUI()
        {
            ...
            if (Main.Settings.ShowOverlay)
            {
                EnsureStyles();
                string error = string.IsNullOrEmpty(AudioSyncRuntime.LastError) ? "-" : AudioSyncRuntime.LastError;
                string text =
                    "ADOFAI AudioSync v0.9.14  [Ctrl+F8 Gate / Ctrl+F9 Overlay]" + Environment.NewLine +
